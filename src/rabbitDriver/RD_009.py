...
"""
    ...
    Model name : reward_RD_009 [Family: RabbitDriver, Gene: SmartCenter]
...
def reward_function(params):
    ...
    try:
        ...
        # Rewards
        reward_center_m1 = 5
        reward_center_m2 = 0.5
        reward_center_m3 = 1e-2
        reward_center_m4 = 1e-4

        if speed < 0.8:
            # Penalize is almost zero speed
            return 1e-3
        ...
        if distance_from_center <= marker_1:
            reward = reward_center_m1 + (0.6 + speed/SPEED_MAX)
        elif distance_from_center <= marker_2:
            reward = reward_center_m2 + (1 - speed/SPEED_MAX)
        elif distance_from_center <= marker_3:
            reward = reward_center_m3 + pow((1.5 - speed/SPEED_MAX), 4)
        else:
            reward = reward_center_m4
            if speed >= 3 :
                reward += -1e-3
            # use waypoints to finds way back
        ...
        # if
        # TODO:
        #  check for position of car, left or right side, wapoints, P, N1, N{i} which is not in alignment with N1P direction.
        #  reward based on current position, direction and possible change in direction with outlier waypoint.
